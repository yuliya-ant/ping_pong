...
background = transform.scale(image.load('galaxy.jpg'),(1000,win_height))
...
player_person = Player('rocket.png', 10, 500,win_height-130, 110,130)
u_f_o = sprite.Group()
for i in range(6):
...
for i in range(2):
    asteroid1 = Enemy('ufo.png', 3, randint(150,850), -400, 150, 100)
    asteroids.add(asteroid1)
font.init()
font1 = font.Font(None, 36)
font2 = font.Font(None, 180)
...
while game:
    ...
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                if nu_fire < 5 and rel_time==False:
                    player_person.fire()
                    fire.play()
                    nu_fire += 1
                if nu_fire >= 5 and rel_time == False:
    ...
    if not finish:
        ...
        minus_collide = sprite.spritecollide(player_person, u_f_o, 1)
        minus_collide2 = sprite.spritecollide(player_person, asteroids, 1)
        if minus_collide or minus_collide2 or missed >= 4:
            # sprite.spritecollide(player_person, u_f_o, 1)
            # sprite.spritecollide(player_person, asteroids, 1)
            # for i in enumerate(minus_collide):
            #     print(i)
            print('1', 'u_f_o:', minus_collide ,
            'asteroids:', minus_collide2,
            'minus_collide_len:', minus_collide, 
            lifes+1)
            lifes += 1
